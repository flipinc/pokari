    ...
    def decorator(task_function: TaskFunction) -> Callable[[], None]:
        ...
        def wrapper(cfg_passthrough: Optional[DictConfig] = None) -> Any:
            ...
            if cfg_passthrough is not None:
                ...
                # no return value from run_hydra() as it may sometime actually run
                # the task_function multiple times (--multirun)
                _run_hydra(
...
@hydra_runner(
    config_path="/configs/emformer", config_name="/emformer_librispeech_char.yaml"
)
def main(cfg):
