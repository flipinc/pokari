...
All codes are adopted from
https://github.com/tensorflow/text/blob/3ef70300b3987614ec9ccc07ee26aeed7237cf35/tensorflow_text/tools/wordpiece_vocab/wordpiece_tokenizer_learner_lib.py
because tensorflow text does not yet expose APIs for these.
...
def ensure_all_tokens_exist(input_tokens, output_tokens, include_joiner_token, joiner):
...
def generate_final_vocabulary(reserved_tokens, char_tokens, curr_tokens):
    ...
    sorted_tokens = sorted(
        sorted(curr_tokens.items(), key=lambda x: x[0]),
        key=lambda x: x[1],
        reverse=True,
    )
...
def learn_with_thresh(word_counts, thresh, params):
    ...
    curr_tokens = ensure_all_tokens_exist(
        char_tokens, {}, params.include_joiner_token, params.joiner
    )
    ...
    for iteration in range(params.num_iterations):
        ...
        for word, count in word_counts:
            if iteration == 0:
                ...
                split_indices = get_split_indices(
                    word, curr_tokens, params.include_joiner_token, params.joiner
                )
        ...
        curr_tokens = ensure_all_tokens_exist(
            char_tokens, next_tokens, params.include_joiner_token, params.joiner
        )

    vocab_words = generate_final_vocabulary(
        params.reserved_tokens, char_tokens, curr_tokens
    )
...
def learn(
    ...
    upper_search, lower_search = get_search_threshs(
        word_counts, params.upper_thresh, params.lower_thresh
    )
    all_counts = get_input_words(
        word_counts, params.reserved_tokens, params.max_token_length
    )
    allowed_chars = get_allowed_chars(all_counts, params.max_unique_chars)

    filtered_counts = filter_input_words(
        all_counts, allowed_chars, params.max_input_tokens
    )

    vocab = learn_binary_search(filtered_counts, lower_search, upper_search, params)

    return vocab


# These are manually added functions to fit with pokari text_featurizer
...
def build_from_corpus(
    ...
    vocab = learn(
        ...
        max_unique_chars=max_corpus_chars,
    )
...
# a property vocab_size needs to be added for text_featurizer, and even though
# _get_vocab_and_ids and detokenize is merged into WordpieceTokenizer in nightly
# version of tensorflow_text, not in v2.4.3
...
class WordpieceTokenizer(tft.WordpieceTokenizer):
    ...
    def _get_vocab_and_ids(self):
        ...
        vocab = check_ops.ensure_shape(
            vocab,
            [
                None,
            ],
        )
        ids = check_ops.ensure_shape(
            ids,
            [
                None,
            ],
